...
## 7. 【改訂】ユーザー症状の精密化を受けた診断の修正(2026-05-16 追記)

ユーザーから、メッシュずれの幾何学的性質について精密な観察が提供された。
これは §3 の判別を**決定的に変える**ため、診断を改訂する。

### 7.1 精密化された症状

- 当初症状: メッシュが顔から数センチ浮く。**メッシュは常に顔の正面**に
  あり、顔が回転すると**より大きい回転半径**で回る。**正面フレームでも
  浮いている。**
- ユーザーの明示: カメラ系の平行移動ではない。**頭部ローカル座標系で
  +z(鼻方向)に一定平行移動**している、という意味なら正しい。
- 後続修正後: 距離は縮むが埋まらず、頭部ローカルの上下(y)ずれも出現
  (当初から在った可能性あり)。別の修正では頭部ローカルの x 軸回転
  (pitch)が乗った状態で顔に追従。
- **すべて定常的**。特定フレーム・特定状況限定ではない。

幾何学的に言えば: メッシュ = 実顔を**頭部ローカルの定数ベクトル `d`**
だけずらしたもの。回転 `R` は実顔と共有。カメラ系では ずれ = `R·d`
(`d` 定数)= **頭部固定の定数オフセット**。
...
| 機構 | ずれの性質 | 観察と整合 |
|---|---|---|
| 候補②(`_recenter` の `R_ref` 欠落) | `(I−R_ref^T)(t_total−t_ref)`。頭部並進依存・ほぼカメラ系 | ✗ 頭部固定の定数でない |
| 候補③(EPnP カメラ深度バイアス) | カメラ z 方向 | ✗ 頭と共に回らない |
| **プラン中核 `(I−R)@J`(関節 pivot ずれ)** | `J−R·J`。**R=I(正面)で厳密に 0**、回転とともに増大 | ✗ **正面で浮く症状を説明不能** |

→ **プランが土台に据えた `(I−R)@J` 機構は、正面フレームでゼロになるため、
正面でも存在する一定の浮きの主因たり得ない。** `(I−R)@J` は実在するが
pose 依存の二次項。候補②③も頭部固定の定数を生まない。§2.1 で「global
slot で浮いた真因は並進補正の不在」と書いたが、**より正確には『並進補正の
不在(=pose 依存成分)』に加えて『頭部固定の定数 `d`(=テンプレート
不一致)』が重畳しており、後者が正面でも残る主成分**である。

### 7.3 残る唯一の説明 — テンプレート/対応の幾何的不一致

`R·d`(頭部固定の定数)が出るのは、**EPnP が pose を合わせ込む 3D
テンプレートと、renderer が実際に描く FLAME メッシュが、頭部/canonical
座標系で定数 `d` だけ食い違う**ときのみ。EPnP は自テンプレートを顔に
ピン留めするので、renderer が別メッシュを描けば差 `d` が `R·d` として
浮く。発生源(優先度順):

1. **EPnP テンプレートと renderer メッシュが別 FLAME ファイル** 【HIGH】
   — §2.2 の「整合リスク」を**主犯候補に格上げ**。object_points は
   `generic_model.pkl`(correspondence.py)、renderer は `flame2020.pkl`
   (scene/flame.py)。
2. **MediaPipe/dlib→FLAME バリセントリック対応資産**が実アバター FLAME
   表面から系統的にずれた点を指す(MICA embedding は特定 topology 用)。
   深度方向のずれ → 頭部ローカル +z の浮き。【MEDIUM-HIGH】
3. **scale 不整合**(flame_scale / shape サイズ)。顔ランドマーク重心は
   FLAME 原点から +z≈0.046m 離れる(landmark 資産で確認)ため、原点
   まわりの scale 誤差は重心を +z にずらす。【MEDIUM】
4. **一定の expression / pose-blendshape ベースライン差**(被写体安静時の
   顔 ≠ FLAME neutral、EPnP は expression=0 テンプレート)。【LOW-MEDIUM】

後続修正で「距離は縮むが埋まらず、y ずれと pitch が露出」したのも整合:
joint 補正・offset-A が pose 依存成分と clip 平均を除いた結果、**残った
頭部固定の定数 `d`(z 主体、y・pitch 成分も含む)が露わになった**。
頭部ローカルの一定 pitch は「テンプレートが実メッシュに対し回転して
傾いている」= 同じ根本原因(テンプレート不一致)の回転版。

### 7.4 GPU 不要の主因確定検証

レンダリング前に、**FLAME canonical 座標で 2 つを直接引き算**する:
- EPnP の `object_points`(`build_shape_aware_landmarks` の出力)
- renderer 自身の `flame2020.pkl` + `J_regressor` で、同じバリセントリック
  対応を使って再構成したランドマーク頂点位置

差がほぼ一定ベクトルなら、それが `d`(主因確定)。reprojection 検証は
**正面フレーム**で行えば `(I−R)@J`・候補②③が最小化され `d` を単離できる。

### 7.5 改訂後の優先順位

1. **§7.4 のテンプレート差分検証を最優先**で実施し、`d` の有無・大きさ・
   方向を確定する。
2. `d` が確認されたら、修正は「EPnP object_points を renderer と同一の
   FLAME メッシュ・同一ランドマーク定義から作る」こと。neck-slot プラン
   とも `_recenter` の `R_ref` 修正(候補②)とも**独立した、別レイヤの
   修正**である。
3. neck-slot プラン・候補② 修正は、`d` を消した後に残る pose 依存成分
   (大ヨー時に増える分)に対して依然有効。ただし**主症状(正面でも残る
   定数浮き)はそれらでは消えない** — プランだけ実装しても解決しない。
